...
const lastfm_apiurl = `https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=${lastfm_username}&api_key=${lastfm_apiKey}&format=json&limit=8`;
  ...
  try { // Sometimes there's API errors and it's pretty swag to catch those
    ...
    for (let i = 0; i < 8; i++) { // Loops for the amount of tracks in that JSON file, increments i
      if (data.recenttracks.track[i].image.length > 1) { // Runs if there is an image in that track's data
        trackimgs += `<a href="${data.recenttracks.track[i].url}"> <img width="60px" alt="${data.recenttracks.track[i].artist} = ${data.recenttracks.track[i].songTitle}" src="${data.recenttracks.track[i].image[1]["#text"]}"></a> `; // Appends track i's image and link to trackimgs 
      }
    }
 trackimgs += `<br>`
  } catch (error) {
...
document.addEventListener("mouseover", function(event) {
  if (event.target.tagName === "IMG") {
      document.getElementById("currentselect").innerHTML = `<img alt="<img src='media/pointer.png'>" src='media/pointer.png'>: ` + event.target.alt + "<br>"
  }
});
