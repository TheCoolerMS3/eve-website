...
const lastfm_apiurl = `https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=${lastfm_username}&api_key=${lastfm_apiKey}&format=json`;
...
async function getSplashVariable() {
  try {
    ...
    for (let i = 0; i < data.recenttracks.track.length; i++) {
      if (data.recenttracks.track[i].image.length > 1) { 
        trackimgs += `<img src="${data.recenttracks.track[i].image[1]["#text"]}"> `;
      }
    }
    trackimgs += "</marquee"
  } catch (error) {
  ...
  setTimeout(function() {document.getElementById("trackimgs").innerHTML = trackimgs;}, 1500);
}
